...
//    cvDilate(tmpmask,pushmask,NULL,8/DOWN_SAMPLE_SIZE);
    ...
    for(int i=0; i<his_frame_buf[0]->width*his_frame_buf[0]->height;i++){
    	...
    	if(v1>threshold && v1<threshold*3)
    		*(push_val+i) = 255.0;
    	else
    		*(push_val+i) = 0;
    	//printf("v1 is %lf", v1);
...
int FingerTracker::getSeeds(){
    ...
    if(contours){
    	...
    	if(count==1 && matchRect(&rect, &prev_rect)){
    		if(rect.width*rect.height > HAND_SIZE/DOWN_SAMPLE_SIZE){ //ignore small blocks
    			prev_rect = rect;
    			stable_counts++;
    		}
    	}else{
...
void FingerTracker::locateTrackPoint(int *result, int box_size){
	...
	unsigned short *tmp_depth_ptr = (unsigned short *)tmp_depth->imageData;
...
//	}
	...
	for(int y=-box_width; y<box_width; y++){
		for(int x=-box_width; x<box_width; x++){
			int pos = (m_x+x)+(y+m_y)*widthStep;
			if(pos >= img_size || pos<0)
				continue;

			unsigned short edge_depth = *(original_frame_ptr+pos)/16;
			//compute the neighbour and see if we are an edge
			int sum = 0;
			for(int i=-1; i<=1; i++){
				for(int j=-1; j<=1; j++){
					int k = (m_x+x+i)+(m_y+y+j)*widthStep;
					if(k >= img_size || k<0)
						continue;
					//int value = *(original_frame_ptr+k);
					int mask = *(handmask_ptr+k);
					sum += mask;
				}
			}

			//label the incomplete blobs, i.e., has less than 6 complete boxes
			if(sum > 1 && sum < 8){
				//edge
				*(tmpmask_ptr+pos) = edge_depth;
			}
		}
	}
...
#ifdef SHOWIMAGE
	cvShowImage("HAND MASK FROM REGION GROWING", handmask);
	cvShowImage("EDGE MASK", tmpmask);
...
int FingerTracker::growHand(int delta, int depth_bound, int box_size){
	...
	int delta_const = delta*delta*255;
	int depth_bound_2 = depth_bound*depth_bound;
...
//
//	int has_matches = 0;
	if(ref_z == 0 || (z-ref_z)*(z-ref_z) >  depth_bound_2){
		for(int i=0; i<box_size*box_size; i++){
			...
			int f = frame_ptr[(c_x) + (c_y)*widthStep];
			if((f-z)*(f-z) < depth_bound_2 && f > 0){
				x = c_x;
				y = c_y;
				ref_z = f;
				z=f;
				//printf("Fixing the depth %d %d %d\n", x, y, ref_z);
				break;
			}
		}
	}
...
//		return 1;
	...
	for(int i=0; i<box_size*box_size; i++){
		...
		//check if any of the points are within the estimated bound
		if((ref_z-c_z)*(ref_z-c_z)<depth_bound_2){
			has_connection++;
		}
		//debug
//		if(i%box_size==0){
...
//					continue;
				...
				if((f-ref_z)*(f-ref_z) > depth_bound_2)
					continue;
				//if the distance is within the delta and also within the set
				//we add a vote
				if(f1*f1*z1 < delta_const && z1 > 0){
					vote++;
				}
			}
		}
...
#endif
	if(size<box_size*box_size*0.01 || has_connection == 0)
		return 1;
...
void FingerTracker::blobTracking(unsigned short *raw_depth){
	...
	cvResize(depth_map, tmp_depth, CV_INTER_AREA); //overwrite the first buffer

	//spatial filter for noise reduction
	cvSmooth(tmp_depth, his_frame_buf[0], CV_MEDIAN, 3, 3);
	...
	if(!hasSeeds){
		...
		if(error){
			...
			locateTrackPoint(xyz, box_size/DOWN_SAMPLE_SIZE);
		}
